...
namespace cmd
{
  public class Dir: CommandBase 
  {
    ...
    private static async Task<long> GetDirSize(string? dirPath)
    {
      ...
      long dirSize = await Task.Run(() => dirInfo.EnumerateFiles(
        "*", 
    ...
    public override void Execute(string[] targets, string[] args, string[] stdout)
    {
      ...
      for (int i = 0; i < targets.Length; i++)
      {
        ...
        foreach (string item in items)
        {
          ...
          bool isDirectory = fileInfo.Attributes.HasFlag(
          ...
          long size = isDirectory 
            ? GetDirSize(item).GetAwaiter().GetResult() : fileInfo.Length;
